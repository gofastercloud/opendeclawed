...
- **Security-First Design**
  ...
  - Self-hosted web search (SearXNG) — agents search the web via a private metasearch engine, no data sent to third-party search APIs
  - Local inference support (Ollama) — chat and embeddings via local models, with LiteLLM abstracting the backend
...
└─────────┬──────────────────────────────────────┬────────────────────┘
          │                                      │
┌─────────▼──────────┐   ┌──────────────────────▼────────────────────┐
│  litellm           │   │  searxng (metasearch)                     │
│  (LLM router)      │   │  JSON API for agent web search            │
│  Internal + Egress │   │  Internal + Egress                        │
│  → Ollama / APIs   │   │  → Google, Bing, DuckDuckGo, etc.        │
└────────────────────┘   └───────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────┐
│  blocky (DNS firewall, DoH + threat blocklists)                     │
└──────────────────────────────────────────────────────────────────────┘
...
- Ollama (recommended) for local inference — install via https://ollama.com or `scripts/setup-ollama.sh`
...
- `SEARXNG_IMAGE`: SearXNG container image
- `SEARXNG_SECRET_KEY`: Instance secret (generated by setup.sh)
- `SEARXNG_PORT`: Internal port (default: 8080, no host binding)
- `SEARXNG_CPUS`, `SEARXNG_MEM`: Resource limits

**Local Inference (Ollama)**
- Ollama runs on the host (not in Docker) — install via `scripts/setup-ollama.sh`
- LiteLLM routes to Ollama via `host.docker.internal:11434`
- Models: `qwen3:8b` (chat), `nomic-embed-text` (embeddings)
...
- litellm, searxng, openclaw-gateway, blocky, docker-socket-proxy, watchtower, dozzle, cloudflared, tailscale, openclaw-cli
- No internet access (internal: true)
- Services communicate on private network only

**openclaw-egress** (bridge):
- openclaw-gateway, searxng, blocky, cloudflared, tailscale, dozzle, openclaw-cli
...
- **65534:65534** (nobody:nogroup): litellm, cloudflared, blocky, dozzle, searxng
...
Agents search the web via a self-hosted SearXNG instance exposed as a JSON API on the internal network. The built-in `web_search` tool is denied; agents use a workspace skill that queries SearXNG instead.

- SearXNG aggregates results from Google, Bing, DuckDuckGo, Wikipedia, Brave, and other engines
- All search traffic routes through blocky (DNS-filtered) and the egress firewall
- No API keys needed for search — SearXNG scrapes public search engines directly
- Configuration: `examples/searxng-settings.yml`

## Local Inference (Ollama)

LiteLLM routes to Ollama running on the host via `host.docker.internal:11434`, providing local chat and embedding inference without external API calls.

- Setup: `scripts/setup-ollama.sh` installs Ollama and pulls default models
- Models: `qwen3:8b` (chat/reasoning), `nomic-embed-text` (768-dim embeddings for memory search)
- Swap models by editing `litellm_config.yaml` — no gateway restart needed
...
Check that the blocky allowlist includes search engine domains. Verify SearXNG health:

```bash
docker exec opendeclawed-gateway curl -s 'http://searxng:8080/search?q=test&format=json' | jq '.results | length'
```

### Ollama not reachable from LiteLLM
...
Confirm that `extra_hosts: ["host.docker.internal:host-gateway"]` is set on the litellm service in `docker-compose.yml`. Test connectivity from inside the container:
