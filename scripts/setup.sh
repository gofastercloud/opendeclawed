...
# Check if a key exists in .env (regardless of its value).
# Usage: env_has KEY
env_has() {
...
# On re-runs, skip credential prompts for optional secrets that were
# already offered (user chose to skip them). We track this by checking
# whether the required key (ANTHROPIC_API_KEY) is already in .env —
# if it is, the credential step has been completed before.
...
if [ "${CREDENTIALS_COMPLETE}" != true ]; then
    ...
    dim "VirusTotal API key: already set (${#VIRUSTOTAL_API_KEY} chars)"
fi

# ── Ingress ──
echo ""
echo -e "  ${BOLD}── Ingress mode ──${NC}"
if env_has INGRESS_MODE; then
    # Re-run: ingress mode already configured in .env — respect it.
    # Auto-detect from tokens as a fallback for legacy .env files.
    if [ -n "${CLOUDFLARE_TOKEN}" ]; then INGRESS_MODE="tunnel"; fi
    if [ -n "${TS_AUTHKEY}" ]; then INGRESS_MODE="tailscale"; fi
    info "Ingress: ${INGRESS_MODE} (from .env)"
    # Tunnel token is generated by setup-cloudflare.sh — don't require it here.
    # Tailscale auth key is user-provided, so prompt if missing.
    ...
    case "${ingress_choice}" in
        2|tunnel)
            INGRESS_MODE="tunnel"
            dim "After setup completes, run: ./scripts/setup-cloudflare.sh"
            dim "This creates the tunnel, DNS, and Cloudflare Access policy."
            ;;
        3|tailscale)
    ...
    # Non-interactive, no INGRESS_MODE in .env: detect from tokens.
    if [ -n "${CLOUDFLARE_TOKEN}" ]; then INGRESS_MODE="tunnel"; fi
    if [ -n "${TS_AUTHKEY}" ]; then INGRESS_MODE="tailscale"; fi
...
if [ -n "${TELEGRAM_BOT_TOKEN}" ]; then
    dim "Telegram bot token: already set (${#TELEGRAM_BOT_TOKEN} chars)"
elif [ "${INTERACTIVE}" = true ]; then
    echo ""
...
# ── Telemetry (opt-in) ──
if env_has TELEMETRY_ENABLED; then
    dim "Telemetry: ${TELEMETRY_ENABLED} (from .env)"
elif [ "${INTERACTIVE}" = true ]; then
    echo ""
...
if docker image inspect "${OPENCLAW_IMAGE}" >/dev/null 2>&1; then
    info "OpenClaw image: ${OPENCLAW_IMAGE}"
elif env_has OPENCLAW_IMAGE; then
    # Image configured in .env but not present — pull silently.
    info "Pulling ${OPENCLAW_IMAGE}..."
    docker pull "${OPENCLAW_IMAGE}" || warn "Pull failed — image may need to be built locally."
else
...
# Only offer on first run (no existing openclaw.json).
if [ "${INTERACTIVE}" = true ] && [ ! -f "${OPENCLAW_JSON}.bak" ] && [ ! -f "${CONFIG_DIR}/devices/paired.json" ]; then
    echo ""
    ...
    if [[ "${run_onboard}" =~ ^[Yy] ]]; then
        ...
        # Clean up onboarding containers but preserve cloudflared if tunnel is active.
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q opendeclawed-cloudflared; then
            docker compose -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" \
                rm -sf openclaw-gateway litellm blocky egress-firewall socket-proxy watchtower dozzle 2>/dev/null || true
        else
...
if [ -f "${LITELLM_CONFIG}" ]; then
    # Re-run: keep existing config without prompting.
...
# ── Auto-create Cloudflare tunnel if needed ──
# If tunnel mode is selected but no token exists yet, and the Cloudflare
# API vars are present, run setup-cloudflare.sh to create the tunnel.
if [ "${INGRESS_MODE}" = "tunnel" ] && [ -z "${CLOUDFLARE_TOKEN:-}" ]; then
    if [ -n "${CF_API_TOKEN:-}" ] && [ -n "${CF_ACCOUNT_ID:-}" ] && \
       [ -n "${CF_ZONE_ID:-}" ] && [ -n "${CF_DOMAIN:-}" ] && [ -n "${CF_SUBDOMAIN:-}" ]; then
        info "No tunnel token found — creating Cloudflare tunnel automatically..."
        "${SCRIPT_DIR}/setup-cloudflare.sh"
        # Reload .env to pick up the new CLOUDFLARE_TOKEN and tunnel vars
        while IFS='=' read -r key value; do
            if [[ "${key}" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then
        ...
        info "Tunnel token loaded from setup-cloudflare.sh"
    else
        warn "Tunnel mode selected but no CLOUDFLARE_TOKEN and missing CF_* API vars."
        warn "Set CF_API_TOKEN, CF_ACCOUNT_ID, CF_ZONE_ID, CF_DOMAIN, CF_SUBDOMAIN in .env"
        warn "Then run: ./scripts/setup-cloudflare.sh"
        warn "Starting without tunnel profile for now."
        # Remove tunnel profile so the stack starts without cloudflared
        PROFILES=()
    fi
fi

# If cloudflared is already running with a valid tunnel, preserve it.
# Only recreate non-tunnel services to avoid disrupting the ingress.
TUNNEL_RUNNING=false
if [ "${INGRESS_MODE}" = "tunnel" ] && \
   docker ps --format '{{.Names}}' 2>/dev/null | grep -q opendeclawed-cloudflared; then
    TUNNEL_RUNNING=true
    info "Cloudflare tunnel already running — preserving it"
fi
...
echo "    Restart: docker compose restart          (preserves tunnel)"
echo "    Stop:    docker compose down              (tears down everything incl. tunnel)"
else
echo "    Stop:    docker compose down"
fi
echo "    Re-run:  ./scripts/setup.sh               (idempotent, preserves tunnel)"
